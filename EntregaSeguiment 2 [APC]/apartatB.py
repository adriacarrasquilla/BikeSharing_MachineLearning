...
class RegressorO(object):
    def __init__(self, arrayTheta, alpha, x, y):
    ...
    def __update(self, hy, epsilon):
        # actualitzar aqui els pesos donada la prediccio (hy) i la y real. pass
        #loss = hy - y
        #cost = np.sum(loss**2)/(2*y.shape[0])
        #gradient = np.dot()
        self.arrayTheta = self.arrayTheta*(1-self.alpha*(epsilon/self.x.shape[0])) -(1/self.y.shape[0])*alpha*( self.x.T.dot((hy - self.y)))

    def train(self, max_iter, epsilon):
        # Entrenar durant max_iter iteracions o fins que la millora sigui inferior a epsilon
        for i in range(max_iter):
            pred = self.predict()
            self.__update(pred, epsilon)
...
#regr = Regressor(thetas,alpha,X_b,y)
#print(regr.train(it,0.1))

thetas = Regressor(X_b,y,thetas,it,epsilon,alpha)
print(thetas)
